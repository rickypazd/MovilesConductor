            ...
            if(success == null) {
                ...
                try {
                    if(!success.isEmpty()){
                        ...
                        if(obj.getString("id_face").length()>0){
                            //cargar_img_face
                            String id_face=obj.getString("id_face");
                            String url="https://graph.facebook.com/" +id_face+"/picture?type=large";
                            new AsyncTaskLoadImage(fotoConductor).execute(url);
                        }
                        if(obj.getString("id_gmail").length()>0){
                            //cargar_img_gmail
                            String url="https://pikmail.herokuapp.com/"+obj.getString("id_gmail")+"?size=100";
                            new AsyncTaskLoadImage(fotoConductor).execute(url);
                        }
                    }else{
                        Toast.makeText(Detalle_viaje_Cliente.this, "Hubo un error al conectarse al servidor.", Toast.LENGTH_SHORT).show();
